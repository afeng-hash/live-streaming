package com.afeng.live.living.provider.config;
...
public class RefreshLivingRoomListJob implements InitializingBean {
    ...
    public void afterPropertiesSet() throws Exception {
        ...
        //一秒钟刷新一次直播间列表数据
        schedulePool.scheduleWithFixedDelay(new RefreshCacheListJob(), 3000, 3000, TimeUnit.MILLISECONDS);
    }
    ...
    class RefreshCacheListJob implements Runnable {
        ...
        public void run() {
            ...
            //这把锁等它自动过期
            boolean lockStatus = redisTemplate.opsForValue().setIfAbsent(cacheKey, 1, 2, TimeUnit.SECONDS);
            if (lockStatus) {
