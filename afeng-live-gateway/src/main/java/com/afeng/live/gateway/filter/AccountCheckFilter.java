...
import com.afeng.live.gateway.properties.GatewayApplicationProperties;
...
public class AccountCheckFilter implements GlobalFilter, Ordered {
    ...
    public Mono<Void> filter(ServerWebExchange exchange, GatewayFilterChain chain) {
        //获取请求url，判断是否为空，为空则返回请求
        //根据url，判断是否存在url白名单中，如果存在，则直接通过
        //不存在白名单中，则提取cookie，并进行校验
        //token 校验，合法则将userid放到下游中
        //获取请求url，判断是否为空，如果为空则返回请求不通过
        ServerHttpRequest request = exchange.getRequest();
        String reqUrl = request.getURI().getPath();
        ServerHttpResponse response = exchange.getResponse();
        HttpHeaders headers = response.getHeaders();
//        headers.add(HttpHeaders.ACCESS_CONTROL_ALLOW_ORIGIN, "http://web.afeng.live.com:5500");
...
//        headers.add(HttpHeaders.ACCESS_CONTROL_MAX_AGE, MAX_AGE);
        ...
        Long userId = accountTokenRPC.getUserIdByToken(afengTokenCookieValue);
