...
import com.afeng.live.gift.constants.SendGiftTypeEnum;
...
public class SendGiftConsumer implements InitializingBean {
    ...
    public void afterPropertiesSet() throws Exception {
        ...
        mqPushConsumer.setConsumerGroup(rocketMQConsumerPropeties.getGroupName() + "_" + SendGiftConsumer.class.getSimpleName());
        ...
        mqPushConsumer.setMessageListener((MessageListenerConcurrently) (msgs, context) -> {
            for (MessageExt msg : msgs) {
                ...
                if (!lockStatus) {
                    //代表曾经消费过
                    //因为是一次型拉取十条进行消费,如果说前九条都是消费成功的,然后第十条消费失败,返回RECONSUME_LATER
                    //那么mq就又会将这十条消息进行重试,那么前面九条就重复消费了
                    //这里用redis来记录这个消息是否被消费过,消费过就跳过
                    //即使下面的 consumeForSendGift 方法执行失败,那么redis还是会记录
                    //但是用户的钱是不会变得,只是mq不会重试,而是跳过
                    continue;
                }
                ...
                AccountTradeRespDTO tradeRespDTO = afengCurrencyAccountRpc.consumeForSendGift(tradeReqDTO);
                //如果余额扣减成功
                ...
                if (tradeRespDTO.isSuccess()) {
                    ...
                    if (SendGiftTypeEnum.DEFAULT_SEND_GIFT.getCode().equals(sendGiftType)) {
                        ...
                        LOGGER.info("[SendGiftConsumer] send gift success,msg is {},receiverId is {}", msg, receiverId);
                    } else if (SendGiftTypeEnum.PK_SEND_GIFT.getCode().equals(sendGiftType)) {
                        //pk类型的送礼，礼物特效全直播间可见，进度条全直播间可见
                        pkImMsgSend(jsonObject,sendGiftMq,sendGiftMq.getReceiverId());
                        LOGGER.info("[SendGiftConsumer] pk send gift success,msg is {},receiverId is {}", msg, receiverId);
                    }
                } else {
                    //利用im将发送失败的消息告知用户
