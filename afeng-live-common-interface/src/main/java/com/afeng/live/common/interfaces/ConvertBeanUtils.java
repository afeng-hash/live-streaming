package com.afeng.live.common.interfaces;
...
public class ConvertBeanUtils {
     ...
     * @return
     */
    public static <T> T convert(Object source, Class<T>
            targetClass) {
        if (source == null) {
            return null;
        }
    ...
    /**
     * 将 List 对象转换成目标对象，注意实现是 ArrayList
     *
     * @param targetClass
     * @param <K>
     * @param <T>
    ...
    public static <K, T> List<T> convertList(List<K> sourceList,Class<T> targetClass) {
        ...
        List targetList = new
                ArrayList((int)(sourceList.size()/0.75) + 1);
        for (K source : sourceList) {
            targetList.add(convert(source, targetClass));}
        return targetList;
    }
    private static <T> T newInstance(Class<T> targetClass) {try {
